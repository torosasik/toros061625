  ...
  Alpine.data('xVariantSelect', (
    ...
    _updateMedia() {
      if (this.currentVariant && this.currentVariant.featured_image != null && !chooseOption && !productBundle) {
        ...
        if (splideEl && !splideEl.dataset.mediaWithVariantSelected) {
    ...
    _updateMultiMediaWithVariant() {
      ...
      var optionConnect = this.mediaOption + '_' + optionValue;
    ...
    handleText(someString) {
      if (someString) {
        return someString.toString().replace('ı', 'i').replace('ß', 'ss').normalize('NFD').replace('-', ' ').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, "_");
      }
    },
    goToFirstSlide() {
      let splideEl = document.getElementById("x-product-" + sectionId);

spreadrWidgetCode = function () {
  ...
  $(function () {
    ...
    var appearance = "";
    var tags =
      '{"comtag":"americantiled-20","intag":"","catag":"","uktag":"","detag":"","frtag":"","estag":"","cntag":"","ittag":"","jptag":"","mxtag":"","brtag":"","autag":"","buttontext":"View on Amazon","islocalizeon":0,"isganalyticson":0,"isfacebookpixel":0}';
    var isActive = "0";
    ...
    if (asins == "") {
      ...
      SetAffiliateTagsSpreadrWidget(
        selectedProduct["region"],
        selectedProduct["amazonid"],
        tagsObj
      );
      if (tagsObj["islocalizeon"] == 1) {
        SetGeoAffiliateTagsSpreadrWidget(
          selectedProduct["amazontitle"],
          tagsObj,
          selectedProduct
        );
      }
      ...
      $(".popupinnercontainer-spreadr").css(
        "background-color",
        appearanceObj["background"]
      );
      $(".introtext-spreadr").css("color", appearanceObj["color"]);

      $(".producttitle-spreadr").css("color", appearanceObj["title"]);

      $(".amazonbutton-spreadr").css(
        "background-color",
        appearanceObj["buttonbackground"]
      );
      $(".amazonbutton-spreadr").css("color", appearanceObj["buttontext"]);

      $(".introtext-spreadr").css(
        "font-size",
        appearanceObj["introsize"] + "px"
      );
      $(".producttitle-spreadr").css(
        "font-size",
        appearanceObj["titlesize"] + "px"
      );
      $(".amazonbutton-spreadr").css(
        "font-size",
        appearanceObj["buttonsize"] + "px"
      );

      $(".introtext-spreadr, .producttitle-spreadr, .amazonbutton-spreadr").css(
        "font-family",
        appearanceObj["fonttype"]
      );
    ...
    $(document).bind("exitintent", function () {
      ...
      if (setting == 1) {
        ...
        if (typeof ga !== "undefined") {
          ga("send", "event", {
            eventCategory: "Spreadr",
            eventAction: "Exit Popup View",
          });
        }
...
function setSpreadrWidgetCookie(cname, cvalue, exdays) {
  ...
  if (oldcookietime != "") {
    ...
    document.cookie =
      "spreadrexitexpiry" + "=" + expires + ";" + expires + ";path=/";
  }
